...
plot1 <- VlnPlot(
  ...
  geom_violin(width = 0.5) +   # narrower violins → more space
...
#| message: false
#| echo: false
genes <- c("FCGBP", "MUC2", "CLCA1", "REG1A", "REG1B", 
           "CEACAM6", "TGFBI","COL1A1", "MMP2", "LYZ", "SPP1", "SAT1")


p_feature_pub <- FeaturePlot(
  Colon,
  features = genes,
...
p_feature_pub <- lapply(p_feature_pub, function(p) {
  p + 
    ggtitle(p$labels$feature) +           
    theme_void() +                       
    theme(
      plot.title = element_text(size = 12, face = "bold", hjust = 0.5),
      legend.position = "right",
...
final_feature_pub <- wrap_plots(p_feature_pub, ncol = 4) + 
